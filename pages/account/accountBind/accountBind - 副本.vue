<template>
	<view class="account-page">
		<view class="page-header">
			...
			<view class="page-desc">{{ desc }}</view>
		</view>
			...
			<view class="list-box">
				<u-cell-group :border="border">
					<view class="panel-main">
						<u-cell-item
							v-if="list.model.mobile == ''"
							title="手机号"
							value="未绑定"
							@click="openPage('bindMobile/bindMobile')"
...
export default {
	...
	methods: {
		...
		mobileClick(index) {
			if (index == 0) {
				this.$u.route({
					url: '/pages/account/updateMobile/updateMobile'
				});
			}
			if (index == 1) {
				this.$u.route({
					url: '/pages/account/unbindMobile/unbindMobile'
				});
			}
		},
		emailAction() {
		...
		emailClick(index) {
			if (index == 0) {
				this.$u.route({
					url: '/pages/account/updateEmail/updateEmail'
				});
			}
			if (index == 1) {
				this.$u.route({
					url: '/pages/account/unbindEmail/unbindEmail'
				});
			}
		},
		weixinAction() {
		...
		weixinClick(index) {
			if (index == 0) {
				this.$u.route({
					url: '/pages/account/updateWeixin/updateWeixin'
				});
			}
			if (index == 1) {
				this.$u.route({
					url: '/pages/account/unbindWeixin/unbindWeixin'
				});
			}
		},
		weiboAction() {
		...
		weiboClick(index) {
			if (index == 0) {
				this.$u.route({
					url: '/pages/account/updateWeibo/updateWeibo'
				});
			}
			if (index == 1) {
				this.$u.route({
					url: '/pages/account/unbindWeibo/unbindWeibo'
				});
			}
		},
		qqAction() {
		...
		qqClick(index) {
			if (index == 0) {
				this.$u.route({
					url: '/pages/account/updateQq/updateQq'
				});
			}
			if (index == 1) {
				this.$u.route({
					url: '/pages/account/unbindQq/unbindQq'
				});
			}
		},
		taobaoAction() {
		...
		taobaoClick(index) {
			if (index == 0) {
				this.$u.route({
					url: '/pages/account/updateTaobao/updateTaobao'
				});
			}
			if (index == 1) {
				this.$u.route({
					url: '/pages/account/unbindTaobao/unbindTaobao'
				});
			}
		},
		alipayAction() {
		...
		alipayClick(index) {
			if (index == 0) {
				this.$u.route({
					url: '/pages/account/updateAlipay/updateAlipay'
				});
			}
			if (index == 1) {
				this.$u.route({
					url: '/pages/account/unbindAlipay/unbindAlipay'
				});
			}
		},
		seted() {
		...
		if (this.vuex_user.hasLogin) {
			...
			api.getAccount(params)
				.then(res => {
					console.log(res);
					if (res.code == 1) {
						this.list.model.mobile = res.data.mobile;
						this.list.model.email = res.data.email;
					} else {
